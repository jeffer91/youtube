...
function normalizarModoAudio(valor) {
  return normalizarTexto(valor, MODO_AUDIO_PREDETERMINADO).toLowerCase();
}
...
function asegurarCarpetasServidor(rutasBase) {
  ...
  [
    rutasBase.raizDatos,
    rutasBase.videosOriginales,
    rutasBase.proyectos,
    rutasBase.temporales,
    rutasBase.subidas,
    rutasBase.videosExportados,
    rutasBase.audiosMejorados
  ].forEach((ruta) => asegurarCarpeta(ruta));
}
...
function crearConfiguracionMulter(rutasBase) {
  const almacenamiento = multer.diskStorage({
    destination(_req, _file, callback) {
      asegurarCarpeta(rutasBase.subidas);
      callback(null, rutasBase.subidas);
    },
    filename(_req, file, callback) {
      callback(null, normalizarNombreTemporal(file.originalname));
    }
  });
  ...
  return multer({
    ...
    fileFilter(_req, file, callback) {
      ...
      if (tipo.startsWith('video/') || permitidas.has(extension)) {
        callback(null, true);
        return;
      }
...
function extraerOpcionesProcesamientoDesdeBody(body = {}) {
  ...
  return {
    ...
    transcripcion: convertirBooleano(body.transcripcion ?? body.crearTranscripcion, true),
    subtitulos: convertirBooleano(body.subtitulos ?? body.agregarSubtitulos, true),
    textosFlotantes: convertirBooleano(body.textosFlotantes ?? body.agregarTextosFlotantes, true),
    cortes: convertirBooleano(body.cortes ?? body.cortarSilencios ?? body.edicionDinamica, true),
    zooms: convertirBooleano(body.zooms ?? body.agregarZooms ?? body.agregarPunchIn, true),
    barraProgreso: convertirBooleano(body.barraProgreso ?? body.agregarBarraProgreso, true),
    etiquetasVisuales: convertirBooleano(body.etiquetasVisuales ?? body.agregarEtiquetasVisuales, true),
    sonidos: convertirBooleano(body.sonidos ?? body.agregarSonidosEdicion, true),
    exportacion: convertirBooleano(body.exportacion, true)
  };
}
...
function normalizarOpcionesDesdeBody(body = {}) {
  ...
  return {
    ...
    usarGemini: opcionesProcesamiento.textosFlotantes ? convertirBooleano(body.usarGemini, false) : false,
    usarFallbackGemini: opcionesProcesamiento.textosFlotantes ? convertirBooleano(body.usarFallbackGemini, true) : false,
...
function crearUrlVideoOriginal(video = {}) {
  if (!video?.rutaOriginal) return null;
  return `/originales/${encodeURIComponent(path.basename(video.rutaOriginal))}`;
}

function crearUrlVideoEditado(resultado = {}) {
  return resultado?.urlPublica || resultado?.exportUrl || null;
}
...
async function crearDiagnosticoSeguro({ guardarReporte = false } = {}) {
  try {
    ...
    return {
      ...
      mensaje: `No se pudo ejecutar el diagnóstico automático: ${error.message}`,
...
async function eliminarTemporalSiExiste(rutaTemporal) {
  ...
  try {
    ...
    console.warn('[Servidor] No se pudo eliminar temporal:', error.message);
  }
}
...
function crearAplicacionExpress({ modoElectron = false } = {}) {
  ...
  app.get('/api/estado', async (_req, res) => {
    ...
    res.json({
      ok: true,
      app: 'AutoVideoJeff',
      estado: diagnosticoEsBloqueante(diagnostico) ? 'SERVIDOR_CON_DIAGNOSTICO_PENDIENTE' : 'SERVIDOR_ACTIVO',
      modo: modoElectron ? 'electron' : 'web',
      predeterminados: {
      ...
      diagnostico,
      rutas: { raizDatos: rutasBase.raizDatos, videosExportados: rutasBase.videosExportados, audiosMejorados: rutasBase.audiosMejorados },
  ...
  app.get('/api/funciones', async (req, res) => {
    try {
      aplicarCabecerasSinCache(res);
      const respuesta = await obtenerFuncionesDisponibles({ busqueda: normalizarTexto(req.query.q, ''), grupoId: normalizarTexto(req.query.grupo, '') });
      res.json(respuesta);
    } catch (error) {
      crearErrorHttp(res, 500, error?.message || 'No se pudo cargar el catálogo de funciones.');
    }
  });

  app.get('/api/funciones/validar', async (_req, res) => {
    try {
      aplicarCabecerasSinCache(res);
      const respuesta = await validarModuloFunciones();
      res.status(respuesta.ok ? 200 : 422).json(respuesta);
    } catch (error) {
      crearErrorHttp(res, 500, error?.message || 'No se pudo validar el módulo de funciones.');
    }
  });

  app.post('/api/funciones/probar', async (req, res) => {
    try {
      aplicarCabecerasSinCache(res);
      const respuesta = await probarFuncionDisponible(req.body || {});
      res.status(respuesta.ok ? 200 : 422).json(respuesta);
    } catch (error) {
      crearErrorHttp(res, 500, error?.message || 'No se pudo probar la función.');
    }
  });

  app.get('/api/funciones/:identificador', async (req, res) => {
    try {
      aplicarCabecerasSinCache(res);
      const respuesta = await obtenerDetalleFuncion(req.params.identificador);
      res.status(respuesta.ok ? 200 : 404).json(respuesta);
    } catch (error) {
      crearErrorHttp(res, 500, error?.message || 'No se pudo leer el detalle de la función.');
    }
  });
  ...
  app.post('/api/procesar-video', upload.single('video'), async (req, res) => {
    ...
    if (!validacionOpciones.ok) {
      return crearErrorHttp(res, 400, validacionOpciones.errores[0] || 'Debes seleccionar al menos una función para procesar.');
    }
    ...
    try {
      ...
      reportarFinalizadoProgreso(jobId, {
        detalle: resultado.mensaje || 'Video procesado correctamente.',
        datos: {
          urlPublica: resultado.resultado?.urlPublica || null,
          nombreExportado: resultado.resultado?.nombreExportado || null,
          exportacion: Boolean(opciones.opcionesProcesamiento?.exportacion)
        }
      });

      return res.json({
        ok: true,
        mensaje: resultado.mensaje || 'Video procesado correctamente.',
        diagnostico,
        jobId,
        resultado: resultado.resultado,
        videoOriginalUrl: crearUrlVideoOriginal(resultado.video),
        videoEditadoUrl: crearUrlVideoEditado(resultado.resultado),
        abrirComparativa: Boolean(opciones.opcionesProcesamiento?.exportacion && resultado.resultado?.ok && !resultado.resultado?.omitido && resultado.resultado?.urlPublica),
        opcionesProcesamiento: opciones.opcionesProcesamiento,
        resumenProcesamiento: resultado.resumenProcesamiento || null,
        detalleProcesamiento: resultado.detalleProcesamiento || null,
        proyecto: resultado.proyecto,
        video: resultado.video,
        entendimiento: resultado.entendimiento,
        audio: resultado.audio,
        transcripcion: resultado.transcripcion,
        edicionDinamica: resultado.edicionDinamica,
...
export async function detenerServidor() {
  ...
  await new Promise((resolve) => {
    instanciaServidor.close(() => resolve());
  });
...
function esEjecucionDirecta() {
  const rutaEjecutada = process.argv[1] ? path.resolve(process.argv[1]) : '';
  return rutaEjecutada === __filename;
}

if (esEjecucionDirecta()) {
  iniciarServidor({ puerto: process.env.PORT || 3000, host: '127.0.0.1', modoElectron: false }).catch((error) => {
    console.error('[Servidor] No se pudo iniciar AutoVideoJeff:', error);
    process.exit(1);
  });
}
