...
function crearPlanPrueba() {
  return {
    ...
    planProduccion: {
      id: 'plan-check-produccion',
      duracionSegundos: 48,
      elementos: [
        { id: 'prod-texto', tipo: 'texto', inicio: 4, fin: 6, nombre: 'Texto plan' },
        { id: 'prod-zoom', tipo: 'zoom', inicio: 8, fin: 10, nombre: 'Zoom plan' }
      ]
    }
  };
}
...
function verificarRutasYConexionesEstaticas() {
  contiene('etapas/03-produccion/procesar-produccion-maestro.service.js', [
    ...
    'editarVideo({ entrada, entendimiento, audio: null, transcripcion, edicionDinamica',
    'prepararSalida({ entrada, entendimiento, audio: null, transcripcion, edicionDinamica, edicion',
    'construirTimelineEditorialProduccion({ plan, edicion, salida, videoBase, edicionDinamica })',
    'puentePlanEdicion',
    'instruccionesEdicionPlan',
    'marcadoresProduccion'
  ]);

  contiene('editar/edicion-dinamica/visual/visual.conexion.js', [
    'generarEventosVisualesDinamicos',
    'construirFiltroVisualFfmpeg',
    'visualesDesdePlan',
    'animacionesRender',
    'eventosVisuales'
  ]);
  ...
  contiene('editar/edicion-dinamica/sonidos/sonidos.conexion.js', [
    'crearEventosSonido({ visualDinamico, edicionDinamica, config, opciones })',
    'eventosSonidoPlan',
    'resumenPlanSonido',
...
function verificarNoErroresConocidos() {
  const ui = leer('app/etapas-ui/produccion-maestro-ui.js');
  exigir(!ui.includes('item.tipo) ? `${item.tipo}s`'), 'Mapa legacy de pistas puede generar animacions/audios. Reemplazar por mapa explícito.');
...
async function verificarFlujoFuncional() {
  ...
  exigir(puente.ok, 'Puente plan-edición no quedó activo.');
  exigir(puente.instrucciones.length >= 8, 'Puente no generó instrucciones suficientes.');
  ...
  const visuales = generarEventosVisualesDinamicos({
    ...
    transcripcion: { textosFlotantes: { textos: [{ inicio: 20, fin: 22, texto: 'Texto automático' }] } },
  ...
  exigir(visuales.ok && !visuales.omitido, 'Visuales dinámicos no quedaron activos.');
  exigir(visuales.planVisual.total >= 5, 'Visuales desde plan no fueron reconocidos.');
  ...
  exigir(animaciones.ok && !animaciones.omitido, 'Animaciones FFmpeg no quedaron activas.');
  exigir(animaciones.filtrosMovimiento.length >= 1, 'Animaciones no generaron filtros de movimiento.');
  exigir(animaciones.filtrosOverlay.length >= 1, 'Animaciones no generaron overlays.');
  ...
  const sonidos = crearEventosSonido({
    ...
    edicionDinamica: { activo: true, puentePlanEdicion: puente, eventosSonidoPlan: puente.eventosSonidoPlan, instruccionesEdicion: puente.instrucciones },
    config: configSonidos,
    opciones: { eventosSonidoPlan: puente.eventosSonidoPlan, instruccionesEdicionPlan: puente.instrucciones }
  });
  exigir(sonidos.ok && !sonidos.omitido, 'Sonidos no quedaron activos.');
  exigir(sonidos.eventosPlanAplicados >= 1, 'No se aplicó ningún SFX del plan.');

  const timeline = construirTimelineEditorialProduccion({
    plan,
    edicion: {
      ok: true,
      visualDinamico: { ok: true, omitido: false, eventosVisuales: visuales.eventos, visualesDesdePlan: visuales.planVisual, animacionesRender: animaciones },
      sonidos: { ok: true, omitido: false, eventosSonido: sonidos.eventos, eventosSonidoPlan: sonidos.eventosPlan, resumenPlanSonido: { total: sonidos.eventosPlan.length, aplicados: sonidos.eventosPlanAplicados, omitidos: 0 } },
      render: { filtroVideo: 'scale=1080:1920' }
    },
    salida: { ok: true, nombreExportado: 'check.mp4', urlPublica: '/exports/check.mp4', ffmpeg: { fallbackVisualUsado: false } },
    videoBase: { multivideoActivo: false, videosFuente: [{ id: 'video-1' }] },
    edicionDinamica: { activo: true, omitido: false, puentePlanEdicion: puente, mapaTiempo: puente.mapaTiempo, eventosVisualesPlan: puente.eventosVisualesPlan, eventosSonidoPlan: puente.eventosSonidoPlan }
  });
  exigir(timeline.ok, 'Timeline editorial no quedó activa.');
  exigir(timeline.marcadores.length >= 10, 'Timeline no generó marcadores suficientes.');
  exigir(timeline.pistas.some((p) => p.id === 'audio-sfx'), 'Timeline no tiene pista audio-sfx.');
  exigir(timeline.pistas.some((p) => p.id === 'zooms'), 'Timeline no tiene pista zooms.');
  ...
  const reporte = await crearReporteFinalEdicion({
    entrada: { proyecto: { id: 'check-produccion-editorial', perfil: '11-contra-11' }, rutas: { carpetaProyecto }, video: { nombreOriginal: 'check.mp4', rutaOriginal: 'check.mp4' } },
    entendimiento: { analisis: { duracionSegundos: 48 }, fotogramas: { fotogramas: [] } },
    audio: null,
    transcripcion: { textosFlotantes: { textos: [{ texto: 'Texto automático', inicio: 20, fin: 22 }] } },
    edicionDinamica: { activo: true, omitido: false, puentePlanEdicion: puente, instruccionesEdicion: puente.instrucciones, eventosVisualesPlan: puente.eventosVisualesPlan, eventosSonidoPlan: puente.eventosSonidoPlan, eventosTextoPlan: puente.eventosTextoPlan, mapaTiempo: puente.mapaTiempo },
    edicion: { ok: true, visualDinamico: { ok: true, omitido: false, eventosVisuales: visuales.eventos, visualesDesdePlan: visuales.planVisual, animacionesRender: animaciones }, sonidos: { ok: true, omitido: false, eventosSonido: sonidos.eventos, eventosSonidoPlan: sonidos.eventosPlan, resumenPlanSonido: { total: sonidos.eventosPlan.length, aplicados: sonidos.eventosPlanAplicados, omitidos: 0 } }, render: { filtroVideo: 'scale=1080:1920' }, salida: { nombreExportado: 'check.mp4' } },
    salida: { ok: true, nombreExportado: 'check.mp4', urlPublica: '/exports/check.mp4', ffmpeg: { fallbackVisualUsado: false }, audio: { tipo: 'seguro' }, render: { planAudio: null } },
    opciones: { perfil: '11-contra-11', plataforma: 'tiktok' }
  });
  exigir(reporte.ok && reporte.reporte?.auditoriaEdicionFinal?.ok, 'Reporte final no generó auditoría válida.');
  exigir(reporte.reporte?.trazabilidadPlan?.planEjecutableDetectado, 'Reporte no detectó plan ejecutable.');
  fs.rmSync(carpetaProyecto, { recursive: true, force: true });

  return {
    instrucciones: puente.instrucciones.length,
    visuales: visuales.eventos.length,
    animaciones: animaciones.eventos.length,
    sfx: sonidos.eventos.length,
...
async function main() {
  verificarPackage();
  verificarRutasYConexionesEstaticas();
  verificarNoErroresConocidos();
