...
async function importarArchivo(rutaRelativa) {
  ...
  try {
    ...
    return {
      ok: true,
      ruta: rutaRelativa,
      exportaciones: Object.keys(modulo || {}),
      error: null
    };
  } catch (error) {
    return {
...
export async function verificarIntegracionFinal({ archivos = ARCHIVOS_INTEGRACION_FINAL } = {}) {
  ...
  return {
    ok: errores.length === 0,
    tipo: 'verificacion-integracion-final',
    archivosRevisados: archivos.length,
    errores,
    advertencias,
    verificaciones,
    mensaje: errores.length === 0 ? 'Integración final cargó correctamente.' : `Integración final con errores: ${errores.join(' ')}`,
