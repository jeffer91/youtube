...
export function registrarRutasModulares(app, opciones = {}) {
  ...
  app.post('/api/proyectos/:proyectoId/transcripciones/:motor/usar', async (req, res) => { try { aplicarCabeceras(res); const seleccion = await seleccionarTranscripcionPrincipalProyecto({ proyectoId: req.params.proyectoId, motor: req.params.motor, motivo: req.body?.motivo || 'ui-entendimiento', usuario: req.body?.usuario || null, permitirVacia: Boolean(req.body?.permitirVacia) }); return responderOk(res, { proyectoId: req.params.proyectoId, seleccion, mensaje: seleccion.mensaje }); } catch (error) { return responderError(res, error, 400); } });
  ...
  app.post('/api/autovideo/sfx/premium/previsualizar', (req, res) => { try { aplicarCabeceras(res); const sfx = previsualizarSfxPremium(req.body || {}); return responderOk(res, { sfx }); } catch (error) { return responderError(res, error, 400); } });
