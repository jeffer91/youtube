/*
  Bloques 6 y 13: Selector Gemini/local + Efectos visuales premium
  Funcion: conectar planificador local/Gemini, capa premium y compilador FFmpeg para entregar un filtro visual final.
*/
...
import { mejorarPlanEfectosPremium } from './premium/index.js';
...
export async function procesarMotorEfectos({ filtroBase = '', entrada = null, entendimiento = null, transcripcion = null, edicionDinamica = null, salida = {}, opciones = {}, progreso = null } = {}) {
  ...
  try {
    ...
    await reportarModulo(progreso, { etapa: 'editar', porcentaje: 84, titulo: 'Planificando efectos premium', detalle: 'Seleccionando efectos visuales por perfil y elevando la capa premium.', archivo: 'editar/efectos/efectos.conexion.js' });

    const planBase = await planificarEfectos({ entrada, entendimiento, transcripcion, edicionDinamica, opciones });
    const plan = mejorarPlanEfectosPremium({ plan: planBase, entrada, entendimiento, transcripcion, edicionDinamica, opciones });
    ...
    await reportarModulo(progreso, { etapa: 'editar', porcentaje: 86, titulo: 'Efectos premium listos', detalle: `${resultado.filtrosAplicados} efectos integrados al render. Premium: ${plan?.premium?.efectosAgregados || 0}.`, datos: resultado.detalle, archivo: 'editar/efectos/efectos.conexion.js' });
    ...
    return crearOmitido({ filtroBase, mensaje: 'No se pudo integrar el motor de efectos premium. Se mantiene el filtro anterior.', error });
  }
}
