...
function crearRespuestaSinMapa({ mensaje, carpetaTiempo }) {
  return {
    ok: true,
    omitido: true,
    etapa: 'edicion-dinamica-tiempo',
    mensaje,
    mapaTiempo: null,
    transcripcionAjustada: null,
    subtitulosAjustados: null,
    textosFlotantesAjustados: null,
    momentosImportantesAjustados: null,
    carpetaTiempo,
    creadoEn: new Date().toISOString()
  };
}

function construirPaqueteTranscripcionAjustado({ transcripcion, ajusteTranscripcion, subtitulosAjustados, textosFlotantesAjustados, geminiAjustado, fallbackAjustado }) {
  if (!transcripcion || ajusteTranscripcion?.omitido) {
    return null;
  }

  const paqueteBase = ajusteTranscripcion.paquete || transcripcion;

  if (!paqueteBase || typeof paqueteBase !== 'object') {
    return null;
  }

  return {
    ...paqueteBase,
    subtitulos: subtitulosAjustados || paqueteBase.subtitulos || null,
    textosFlotantes: textosFlotantesAjustados || paqueteBase.textosFlotantes || null,
    gemini: geminiAjustado || paqueteBase.gemini || null,
    fallback: fallbackAjustado || paqueteBase.fallback || null,
    capasVideo: null,
    edicionDinamica: {
      ...(paqueteBase.edicionDinamica || {}),
      ajustada: true,
      necesitaReconstruirCapasVideo: true,
      motivo: 'Los tiempos fueron ajustados después de cortar silencios.'
    }
  };
}
...
export async function procesarTiempoDinamico({ entrada, entendimiento, transcripcion = null, cortes = null, config, carpetaEdicionDinamica, opciones = {} } = {}) {
  ...
  const mapaTiempo = crearMapaTiempoDesdeSegmentos({
    segmentosConservados: plan.segmentosConservados,
    duracionOriginal: plan.duracionOriginal || entendimiento?.analisis?.duracionSegundos || null,
    duracionEditada: plan.duracionEditada || null,
    cortes: plan.cortes || []
  });
  ...
  const subtitulosAjustados = await ajustarSubtitulosDinamicos({
    subtitulos: transcripcion?.subtitulos || null,
    segmentosAjustados,
    carpetaTiempo,
    opciones
  });

  const textosFlotantesAjustados = ajustarTextosFlotantesDinamicos({
    textosFlotantes: transcripcion?.textosFlotantes || null,
    mapaTiempo
  });

  const geminiAjustado = ajustarMomentosImportantesDinamicos({
    origenMomentos: transcripcion?.gemini || null,
    mapaTiempo
  });

  const fallbackAjustado = ajustarMomentosImportantesDinamicos({
    origenMomentos: transcripcion?.fallback || null,
    mapaTiempo
  });

  const transcripcionAjustada = construirPaqueteTranscripcionAjustado({
    transcripcion,
    ajusteTranscripcion,
    subtitulosAjustados,
    textosFlotantesAjustados,
    geminiAjustado,
    fallbackAjustado
  });
  ...
  const resultado = {
    ...
    omitido: false,
    etapa: 'edicion-dinamica-tiempo',
    mensaje: 'Mapa de tiempo, transcripción, subtítulos y textos ajustados correctamente.',
    mapaTiempo,
    transcripcionAjustada,
    ajusteTranscripcion,
    subtitulosAjustados,
    textosFlotantesAjustados,
    momentosImportantesAjustados: {
      gemini: geminiAjustado,
      fallback: fallbackAjustado
    },
    pendienteAjusteTranscripcion: false,
    config: {
      intensidad: config?.intensidad || null
    },
    opciones: {
      ajustarSubtitulos: true,
      ajustarTextosFlotantes: true,
      ajustarMomentosImportantes: true,
      ...opciones
    },
    carpetaTiempo,
