...
function crearRespuestaOmitida({ mensaje, carpetaCortes, planCortes = null, silencios = [] }) {
  return { ok: true, omitido: true, etapa: 'edicion-dinamica-cortes', mensaje, videoDinamico: null, audioDinamico: null, silencios, planCortes, resumen: { cantidadSilenciosDetectados: silencios.length, cantidadCortesAplicados: 0, segundosEliminados: 0 }, carpetaCortes, creadoEn: new Date().toISOString() };
}
...
export async function procesarCortesDinamicos({ entrada, entendimiento, audio = null, config, carpetaEdicionDinamica, opciones = {} } = {}) {
  ...
  try {
    ...
    if (!rutaVideoOriginal || !rutaAnalisisAudio) return crearRespuestaOmitida({ mensaje: 'Falta video o audio para analizar silencios.', carpetaCortes });
    ...
    if (!validacion.ok || planCortes.cortes.length === 0) return crearRespuestaOmitida({ mensaje: validacion.mensaje || 'No hay cortes seguros para aplicar.', carpetaCortes, planCortes: { ...planCortes, validacion }, silencios });
    const resultadoAplicacion = await aplicarCortesVideo({ rutaVideoOriginal, segmentosConservados: planCortes.segmentosConservados, carpetaCortes, nombreSalida: 'video-sin-silencios.mp4' });
    const resultado = { ok: true, omitido: false, etapa: 'edicion-dinamica-cortes', mensaje: 'Cortes de silencio aplicados correctamente.', videoDinamico: resultadoAplicacion.rutaSalida, audioDinamico: null, silencios, planCortes: { ...planCortes, validacion }, resumen: { cantidadSilenciosDetectados: silencios.length, cantidadCortesAplicados: planCortes.cortes.length, duracionOriginal: planCortes.duracionOriginal, duracionEditada: planCortes.duracionEditada, segundosEliminados: planCortes.segundosEliminados, porcentajeEliminado: planCortes.porcentajeEliminado }, ffmpeg: resultadoAplicacion.ffmpeg, carpetaCortes, creadoEn: new Date().toISOString() };
    await escribirJson(path.join(carpetaCortes, 'resultado-cortes.json'), resultado);
    return resultado;
  } catch (error) {
    if (config?.modoSeguro !== false) return crearRespuestaOmitida({ mensaje: `Modo seguro: no se aplicaron cortes porque ocurrió un error: ${error.message}`, carpetaCortes });
    throw error;
  }
}
