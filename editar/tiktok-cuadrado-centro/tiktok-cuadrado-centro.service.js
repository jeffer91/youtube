...
function seleccionarTranscripcionRender(transcripcion, edicionDinamica) {
  return edicionDinamica?.transcripcionAjustada || transcripcion;
}
...
function construirEdicion({ entrada, entendimiento, opciones, config, medidas, recorte, lienzo, detalleFiltro, capasTranscripcion, visualDinamico, sonidos, nombreExportado, rutaEdicion, nombreArchivoEdicion, videoRender, edicionDinamica }) {
  return {
    ...
    modo: config.modo,
    preset: { nombre: config.nombre, version: config.version, descripcion: config.descripcion, video: config.video, recorte: config.recorte, exportacion: config.exportacion },
    entrada: { rutaVideoOriginal: entrada.video.rutaOriginal, rutaVideoRender: videoRender.ruta, origenVideoRender: videoRender.origen, nombreOriginal: entrada.video.nombreOriginal || null, nombreSeguro: entrada.video.nombreSeguro || null, orientacionDetectada: medidas.orientacionDetectada, medidasOriginales: { width: medidas.anchoOriginal, height: medidas.altoOriginal, fps: medidas.fps, duracionSegundos: medidas.duracionSegundos, relacionAspectoOriginal: medidas.relacionAspectoOriginal }, tieneAudio: medidas.tieneAudio, tieneVideo: medidas.tieneVideo },
    composicion: { estrategia: 'cuadrado-centrado-en-lienzo-vertical', recorte, lienzo, filtro: detalleFiltro.pasos },
    salida: { nombreExportado, extension: '.mp4', formato: config.video.formato, width: lienzo.anchoFinal, height: lienzo.altoFinal, fps: config.video.fps, nombreArchivoEdicion, rutaEdicion },
    render: { rutaVideoEntrada: videoRender.ruta, origenVideoEntrada: videoRender.origen, usarAudioDelVideoRender: videoRender.usarAudioDelVideoRender, rutaAudioConSonidos: sonidos?.audioConSonidos || null, usarAudioConSonidos: Boolean(sonidos?.audioConSonidos && !sonidos?.omitido), filtroVideo: visualDinamico.filtroVideo, filtroVideoBase: detalleFiltro.filtroVideoBase || detalleFiltro.filtroVideo, codecVideo: config.exportacion.codecVideo, codecAudio: config.exportacion.codecAudio, crf: config.exportacion.crf, presetFfmpeg: config.exportacion.presetFfmpeg, audioBitrate: config.exportacion.audioBitrate, pixFmt: config.exportacion.pixFmt },
    transcripcion: { capasAplicadas: Boolean(capasTranscripcion.aplicadas), mensaje: capasTranscripcion.mensaje, capasVideo: capasTranscripcion.capasVideo, ajustadaPorEdicionDinamica: Boolean(edicionDinamica?.transcripcionAjustada) },
    visualDinamico,
    sonidos,
    opciones: { plataforma: opciones?.plataforma || config.plataforma, modo: opciones?.modo || config.modo, mejorarAudio: opciones?.mejorarAudio ?? null, modoAudio: opciones?.modoAudio || null, crearTranscripcion: opciones?.crearTranscripcion ?? null, agregarSubtitulos: opciones?.agregarSubtitulos ?? null, agregarTextosFlotantes: opciones?.agregarTextosFlotantes ?? null, agregarSonidosEdicion: opciones?.agregarSonidosEdicion ?? null, edicionDinamicaActiva: Boolean(edicionDinamica?.activo && !edicionDinamica?.omitido) },
    auditoria: { moduloUsado: 'editar/tiktok-cuadrado-centro/tiktok-cuadrado-centro.service.js', archivoEdicion: nombreArchivoEdicion, rutaEdicion, filtroVideo: visualDinamico.filtroVideo, filtroVideoBase: detalleFiltro.filtroVideoBase || detalleFiltro.filtroVideo, recorteResumen: recorte.explicacion, lienzoResumen: lienzo.resumen, capasTranscripcionAplicadas: Boolean(capasTranscripcion.aplicadas), visualDinamicoAplicado: Boolean(visualDinamico && !visualDinamico.omitido), sonidosAplicados: Boolean(sonidos && !sonidos.omitido) },
    notas: crearNotasEdicion({ recorte, lienzo, capasTranscripcion, visualDinamico, sonidos, videoRender }),
...
export async function crearEdicionTikTokCuadradoCentro({ entrada, entendimiento, audio = null, transcripcion = null, edicionDinamica = null, opciones = {} } = {}) {
  ...
  const sonidos = await procesarSonidosEdicion({ rutaVideoBase: videoRender.ruta, visualDinamico, edicionDinamica, opciones });
